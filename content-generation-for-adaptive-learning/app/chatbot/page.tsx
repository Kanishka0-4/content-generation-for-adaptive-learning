...
export default function ChatbotPage() {
  ...
  async function generateRoadmap() {
    ...
    try {
      ...
      // ✅ Normalize → UI-safe format
      const normalized: RoadmapWeek[] = data.map((w: any, i: number) => ({
  ...
  try {
    const res = await fetch("/api/roadmap/save", {
      ...
      body: JSON.stringify({
        roadmap,
        course_title: input,          // subject name
        total_duration: "8 weeks",    // or extract later
        userId: "demo-user",          // replace with real auth user
      }),
    });
